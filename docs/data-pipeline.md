...
실제 위치는 [`old/`](../old/) 폴더(루트 번호 파일은 legacy 호환 redirect 만 남음). 모두 **브라우저 콘솔 IIFE**(서버 크롤러 아님).

### 1-fetch-ereter.js — ereter.net 추출 ([`old/1-fetch-ereter.js`](../old/1-fetch-ereter.js))
...
### 3-fetch-zasa.js — zasa 비공식 ☆표 ([`old/3-fetch-zasa.js`](../old/3-fetch-zasa.js))
...
### 3-fetch-lv12-batch.js / 3-fetch-lv11-batch.js — 학습용 batch ([`old/3-fetch-lv12-batch.js`](../old/3-fetch-lv12-batch.js))
...
> 루트 [`2-calc-score.js`](../2-calc-score.js) 는 사용자 실행용 호환 redirect(ohsorry.js fetch+eval) — 수집 스크립트 아님. 운영 자동화 대부분은 ohSorryAdmin 으로 이관.
...
### 3.1 런타임 데이터 (gist + 로컬 사본)
| 파일 | top-level | 엔트리 | core 용도 |
|------|-----------|--------|-----------|
| [`ereter-data.json`](../ereter-data.json) | `extractedAt, source, count, playerCount, charts[], players{}` | charts: `{title, diff, level, ec, hc, exh, ec_n, hc_n, exh_n}` / players: `{iidxId: ★}` | ★추정·추천 baseStar·매칭 점수의 정본 |
| [`zasa-data.json`](../zasa-data.json) | `extractedAt, source, count, countByGameLevel{}, countByDiff{}, charts[]` | `{title, diff, gameLevel, level}` | ereter 미등록 차트 곡수 보강(추천/★추정 미사용) |
...
### 3.3 학습/내부 데이터 (배포 안 함)
- [`source/`](../source/)·`source_lv12/` : 학습 데이터(유저별 JSON), [`dataset.json`](../dataset.json)(약 18MB), `loocv-*.json`.
- [`logic/`](../logic/) : 모델 코드 v3.0.2~v3.3.6 사본 + `v3-params.json`.
- [`scripts/`](../scripts/) : 모델 학습/평가/빌드 스크립트(build-v3xx / eval-* / train-loocv 등).
- [`archive/`](../archive/) : 버전별 모듈 사본.

### 3.4 package.json
- `name: ohsorry`, `version: 1.0.0`, `main: 1-fetch-ereter.js`, `type: commonjs`.
