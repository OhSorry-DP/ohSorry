...
window.OhsorryCore = {
  ...
  const CORE_VERSION_SHORT = '0.0.389'.replace(/^0\.0\./, '');  // '389'
  ...
  // songs 캐시 — INF 유저 차트 단위 필터 + (INF/AC 공통) ALL 통계 난이도별 총 채보수 분모 계산에 사용.
  //   AC 유저도 ac & 1 수록 비트 필터가 필요하므로 모든 유저 fetch.
  ...
  if (window.OhsorryDb && typeof window.OhsorryDb.getSongsByNorm === 'function') {
    try {
      ...
      console.warn('[step2] songs cache fetch 실패 (INF 필터 / 난이도별 총 채보수 분모):', err && err.message);
    }
  }
  ...
  // 난이도별 총 채보 수 (ALL 통계 막대 분모) — textage-meta 의 DP 채보 levels × songs.ac/legen 수록 비트.
  //   유저 타입 비트: INF=2 / AC=1. DX(LEGGENDARIA)=legen 컬럼, 그 외(DN/DH/DA/DB)=ac 컬럼.
  //   songs 레코드(동명이곡 = 같은 norm 의 여러 레코드 포함) 각각에 대해 수록 비트가 set 이면 그 채보를 카운트.
  //   songs / textage fetch 실패 시 null → ohsorryRender 가 allCharts 집계로 fallback.
  ...
  if (textageSongs && songsByNorm) {
      ...
      for (const rec of cands) {                 // songs 레코드 각각 카운트
        for (const tk of DP_KEYS) {
          ...
          for (const lv of levelsList) {
            if (typeof lv[tk] === 'number') { lvl = lv[tk]; break; }
          }
    ...
    console.log('[step2] ALL 통계 난이도별 총 채보수 분모:', JSON.stringify(totals));
  }
