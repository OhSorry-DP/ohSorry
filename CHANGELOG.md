...
### 2026-09-19 — 북마클릿 시리즈 수집 0차트 재시도

HTTP 200 응답인데 0차트로 파싱되면 2초·4초·8초 백오프로 최대 3회 재요청한다. 재시도 후에도 0차트면 해당 시리즈에서 수집을 중단하고 사용자에게 재시도를 안내한다.
...
같은 시간대에 크롤된 다른 4명은 전부 정상(426~4,468건)이었고, 재실행하니 1,837건이 들어갔다. 즉 첫 실행만 스코어 단계에서 실패했는데 **그걸 알아챌 방법이 화면에도 로그에도 없었다.** 실패 지점 자체는 첫 실행의 콘솔 로그가 없어 확정하지 못했다.

- 🔴 `uploadResult` 의 결과를 **아무도 확인하지 않았다.** `up.skipped` 만 보고 무조건 완료 박스를 띄워서, 프로필·스코어가 통째로 실패해도 성공과 화면이 **똑같았다.** 이제 `up.profile.ok` / `up.scores` 를 확인하고, 실패면 사유와 함께 `alert` + `console.error` 를 낸다.
- `upsertUserChartScores` 가 **0건 입력에 `{ok:true}`** 를 돌려줘 호출부가 "성공" 으로 로그까지 남겼다. `{ok:true, inserted:0, empty:true}` 로 바꿔 "성공" 과 "쓸 게 없었음" 을 구분한다. 성적을 하나도 못 읽은 크롤이 이제 조용히 지나가지 않는다.
- 프로필 upsert 가 실패하면 `scores` 는 `users` FK 때문에 어차피 전량 롤백된다. 이제 **시도하지 않고 사유를 남긴다.**

조회 실패를 "값 없음" 으로 둔갑시키지 않는다는 2026-09-04 항목과 같은 갈래다 — 이번엔 **"성공+0건" 을 "성공" 으로 뭉갠 것**이었다.
...
- [modules/calcOhsorryCore.js](modules/calcOhsorryCore.js): `loginBtnHtml` / `bindLoginBtns` 를 `window.OhsorryCore` 로 내보낸다. 티켓 발급 로직은 그대로 한 벌이고 부트로더가 재사용한다.
- 버튼 모양을 `variant` 로 분기 — `'compact'`(여러 명 리스트) / `'start'`(시작 모달, 흰 배경이라 초록 아웃라인) / 그 외(완료 박스 전폭).
- [ohsorry.js](ohsorry.js): 프로필 박스 안에 버튼을 넣는다. **본인 프로필의 IIDX ID 로만** 발급한다 — 아래 IIDX 입력칸은 라이벌 ID 로 바꿀 수 있어 신뢰할 수 없다.
