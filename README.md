...
| v3.3.5 (D3) | **채택 분기 재정의 + OSR (v0.0.5) 개선**: OSR135 ≥ 11.0 → 무조건 OSR135 / 10.5~11.0 → OSR135↔group값 선형 보간 (경계 점프 방지) / 그 미만 → group 별 (A·B → OSR, C → oldOSR). OSR (calc-OSRating) v0.0.2 → **v0.0.5**: bandCorr 0.5 bin → 선형 보간, 그룹 A/B/C 경계 30±5 soft transition (29/30 점프 제거), nativeStar HC 표본 부족 시 shrinkage, ASSIST(lamp=2) fitData 제외, M feature 단일최고곡 → top-3 평균 robust 화 + 재학습. supabase 309명: OSR MAE 0.315 → **0.294** (-6.7%), 11~13 영역 전반 개선. ohSorryRating.json 재학습본 (1187 ratings) gist 배포. |
