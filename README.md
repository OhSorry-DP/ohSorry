...
| v3.3.5 (textage) | **textage 채보 메타로 noteCount / missCount 보강** — `textage-meta.json` (채보별 총 노트 수) 을 gist 에서 fetch, `norm(곡명)` 매칭으로 charts 에 `noteCount` 채움. 플레이한 곡은 `missCount = noteCount - pgreat - great` 로 재계산 (eagate 파싱 missCount 보다 신뢰도 ↑ — EX 표기 `2392(986/420)` 의 986=pgreat, 420=great). 결과: **EXH 추천곡의 미스카운트 정렬이 실제로 동작**, supabase `charts_json` 업로드도 INF 처럼 noteCount / missCount 포함 (게스트 서열표 곡 클릭 토스트의 rate 바 등). |
